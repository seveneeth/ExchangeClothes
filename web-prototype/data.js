/* =====================================================
   梦幻衣橱 · 数据层
   所有服饰均为程序化 SVG，根据角色体型锚点动态生成，
   从根本上保证任何体型都贴合、无穿模。
   ===================================================== */

/* ---------- 颜色工具 ---------- */
function shade(hex, p){ // p: -1 变暗 ~ 1 变亮
...
function bodyHalfAt(b, y){ // 躯干在某 y 的半宽（用于条纹等图案贴合）
...
/* ---------- 共享体型轮廓（角色与服装使用同一套函数，保证完美贴合） ---------- */
function armPts(b, side){ // 手臂中心线（肩→肘→腕）
...
function armPtsT(b, side, t){ // 手臂中心线截断到 t∈(0,1]（贝塞尔插值，短袖用）
  const { cx, shoulderY: sy, armX, elbowX, elbowY, wristX, wristY } = b;
...
function legPts(b, side, endY){ // 腿部中心线（髋→踝，可截断）
...
function footCX(b, side){ return b.cx + side * b.footX; } // 脚掌中心 x（脚尖略朝外）
function torsoPath(b, e){ // 躯干完整轮廓（e = 向外扩张量）
  e = e || 0;
...
function torsoTopPath(b, e, hemY){ // 上衣轮廓：躯干轮廓在 hemY 处截平
...
/* ---------- 调色板 ---------- */
const SKINS      = ['#ffe3d0', '#fcd0b0', '#f2b98e', '#dda27a', '#b37a50', '#8a5a38'];
const HAIRCOLORS = ['#2b2b33', '#5b3a29', '#8a5a33', '#c98a3d', '#f0cd7a', '#e87a8f', '#a78bfa', '#5eb8f0', '#7dc9a0', '#e8e4ee'];
const EYECOLORS  = ['#4a3b2a', '#2f6f4f', '#3a6ea8', '#7a4fd0', '#c05a3a', '#d47fa6', '#e0a63c', '#7a828e'];
const EYESHAPES  = [
  { id: 'round',   name: '圆圆眼' },
  { id: 'almond',  name: '杏眼' },
  { id: 'narrow',  name: '丹凤眼' },
...
function pantsBase(b, o){
  ...
  const w = o.slim ? legW + 2 : legW + 4.5;   // 裤管包住腿
  // 髋部（底边与躯干轮廓底弧一致，完全遮住身体）
...
const HAIRSTYLES = {
  ...
  bun: {
    ...
    front(b, c){
      return hairCap(b, c, { drop: 0.03, top: 1.12 });
    }
  },
  buzz: {
    name: '利落寸头',
...
const T = (arr, id) => arr.find(x => x.id === id);

/* 上衣 6 */
I('t1', 'top', '纯白T恤', ['casual', 'basic'], ['#f6f6f8'],
  b => topBase(b, { c: '#f6f6f8', sleeve: 'short', collar: 'round', hemLine: 1 }));
I('t2', 'top', '海洋蓝衬衫', ['casual', 'fresh'], ['#7fb2e8'],
  b => topBase(b, { c: '#7fb2e8', sleeve: 'long', collar: 'v', buttons: 1, hemLine: 1 }));
I('t3', 'top', '奶黄连帽卫衣', ['casual', 'cute'], ['#ffd76e'],
  b => topBase(b, { c: '#ffd76e', sleeve: 'long', collar: 'round', hood: 1, pocket: 1 }));
I('t4', 'top', '薄荷运动背心', ['sporty', 'cool'], ['#7de8c8'],
  b => topBase(b, { c: '#7de8c8', sleeve: 'none', collar: 'v', hemY: b.waistY + 4, hemLine: 1 }));
I('t5', 'top', '樱花粉吊带', ['sweet', 'cute'], ['#ff9dbf'],
  b => topBase(b, { c: '#ff9dbf', sleeve: 'strap', collar: 'round', hemY: b.waistY + 2, hemLine: 1 }));
I('t6', 'top', '海军风条纹衫', ['casual', 'sea'], ['#4a6fa8', '#f6f6f8'],
  b => topBase(b, { c: '#4a6fa8', sleeve: 'short', collar: 'round', stripes: '#f6f6f8' }));

/* 下装 6 */
I('p1', 'bottom', '经典牛仔裤', ['casual', 'basic'], ['#5b84c4'],
  b => pantsBase(b, { c: '#5b84c4', cuffs: 1 }));
I('p2', 'bottom', '卡其短裤', ['casual', 'sporty'], ['#d8b06e'],
  b => pantsBase(b, { c: '#d8b06e', len: 'short' }));
I('p3', 'bottom', '黑色运动裤', ['sporty', 'cool'], ['#3a3a44'],
  b => pantsBase(b, { c: '#3a3a44', slim: 1, belt: '#e86a6a' }));
I('p4', 'bottom', '工装多袋裤', ['cool', 'casual'], ['#8a9a6a'],
  b => pantsBase(b, { c: '#8a9a6a', cargoPockets: 1 }));
I('p5', 'bottom', '蜜桃运动裤', ['cute', 'sporty'], ['#f0a8c0'],
  b => pantsBase(b, { c: '#f0a8c0', cuffs: 1 }));
I('p6', 'bottom', '牛仔热裤', ['cool', 'sea'], ['#6f92cf'],
  b => pantsBase(b, { c: '#6f92cf', len: 'short', cuffs: 1 }));

/* 裙子 5（独立类别） */
I('s1', 'skirt', '学院百褶裙', ['campus', 'sweet'], ['#a8c8f0'],
  b => skirtBase(b, { c: '#a8c8f0', len: 58, flare: 0.5, pleats: 1 }));
I('s2', 'skirt', '绯红A字裙', ['sweet', 'formal'], ['#e86a6a'],
  b => skirtBase(b, { c: '#e86a6a', len: 50, flare: 0.62 }));
I('s3', 'skirt', '莓果格纹裙', ['campus', 'cool'], ['#c46a8a'],
  b => skirtBase(b, { c: '#c46a8a', len: 60, flare: 0.55, plaid: 'rgba(255,240,244,.75)' }));
I('s4', 'skirt', '云朵蛋糕裙', ['cute', 'princess'], ['#ffd0e8'],
  b => skirtBase(b, { c: '#ffd0e8', len: 66, flare: 1.05, hem: 'wave', amp: 7, layers: 1 }));
I('s5', 'skirt', '丹宁牛仔裙', ['casual', 'cool'], ['#6f92cf'],
  b => skirtBase(b, { c: '#6f92cf', len: 46, flare: 0.4, pocket: 1 }));

/* 连衣裙 / 套装 7 */
I('d1', 'dress', '碎花沙滩裙', ['sweet', 'fresh'], ['#ffe8f0', '#ff8fb0'],
  b => dressBase(b, { bodiceC: '#fff0f5', skirtC: '#ffe8f0', dk: '#f3b8cc', len: 96, flare: 0.9, sleeve: 'strap', collar: 'v', flowers: '#ff8fb0', hem: 'wave', amp: 5 }));
I('d2', 'dress', '草莓公主裙', ['princess', 'sweet'], ['#f8b8d8', '#ff7eb6'],
  b => dressBase(b, { bodiceC: '#f8b8d8', skirtC: '#f8c8e0', len: 118, flare: 1.35, sleeve: 'puff', sash: '#ff7eb6', bowKnot: 1, hem: 'wave', amp: 8, waves: 7, layers: 1 }));
I('d3', 'dress', '星夜晚礼服', ['formal', 'elegant'], ['#4a3f78'],
  b => dressBase(b, { c: '#4a3f78', dk: '#372e5e', len: 128, flare: 1.1, sleeve: 'none', collar: 'v', seqDots: 1, slit: 1 }));
I('d4', 'dress', '云锦汉服', ['classic', 'elegant'], ['#f0e0d0', '#c85a6a'],
  b => dressBase(b, { bodiceC: '#f0e0d0', skirtC: '#c85a6a', len: 120, flare: 1.15, sleeve: 'wide', collar: 'v', wrap: '#a83a48', sash: '#e8d5b0' }));
I('d5', 'dress', '赫本小黑裙', ['formal', 'cool'], ['#2e2e38'],
  b => dressBase(b, { c: '#2e2e38', dk: '#211f28', len: 78, flare: 0.72, sleeve: 'none', collar: 'round', sash: '#2e2e38' }));
I('d6', 'dress', '鎏金亮片裙', ['party', 'stage'], ['#ffd76e', '#ff9d6e'],
  b => `<defs><linearGradient id="gseq" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#ffd76e"/><stop offset=".55" stop-color="#ffab6e"/><stop offset="1" stop-color="#ff7eb6"/></linearGradient></defs>` +
    dressBase(b, { bodiceC: 'url(#gseq)', skirtC: 'url(#gseq)', dk: '#e88a4a', len: 92, flare: 0.95, sleeve: 'none', collar: 'round', seqDots: 1 }));
I('d7', 'dress', '星光小礼服套装', ['formal', 'cool'], ['#3d3d6e', '#e86a6a'],
  b => pantsBase(b, { c: '#3d3d6e', slim: 1 }) +
    topBase(b, { c: '#3d3d6e', dk: '#2c2c52', sleeve: 'long', collar: 'v', hemY: b.hipY + 6, buttons: 1 }) +
...
/* 鞋子 6 */
I('sh1', 'shoes', '元气小白鞋', ['sporty', 'casual'], ['#f6f6f8'],
  b => shoesBase(b, { type: 'sneaker', c: '#f6f6f8', accent: '#a78bfa' }));
I('sh2', 'shoes', '绅士小皮鞋', ['formal', 'basic'], ['#4a3b30'],
  b => shoesBase(b, { type: 'leather', c: '#4a3b30' }));
I('sh3', 'shoes', '帅气马丁靴', ['cool', 'casual'], ['#6a4a38'],
  b => shoesBase(b, { type: 'boot', c: '#6a4a38' }));
I('sh4', 'shoes', '夏日凉鞋', ['fresh', 'casual'], ['#c98a3d'],
  b => shoesBase(b, { type: 'sandal', c: '#c98a3d' }));
I('sh5', 'shoes', '樱桃高跟鞋', ['elegant', 'formal'], ['#d84a5a'],
  b => shoesBase(b, { type: 'heels', c: '#d84a5a' }));
I('sh6', 'shoes', '复古帆布鞋', ['campus', 'casual'], ['#e84a5a'],
  b => shoesBase(b, { type: 'canvas', c: '#e84a5a', accent: '#fff' }));

/* 帽子 6 */
I('h1', 'hat', '绯红棒球帽', ['sporty', 'casual'], ['#e86a6a'],
  b => {
    const { cx, cy, hr, hry } = b;
...
I('h2', 'hat', '薄荷毛线帽', ['cute', 'casual'], ['#7de8c8'],
  b => {
    const { cx, cy, hr, hry } = b;
...
I('h3', 'hat', '魔术师礼帽', ['formal', 'cool'], ['#2a2a34'],
  b => {
...
I('h4', 'hat', '艺术家贝雷帽', ['elegant', 'cute'], ['#ff9dbf'],
  b => {
...
I('h5', 'hat', '黄色头巾帽', ['fresh', 'cute'], ['#ffd76e'],
  b => {
    const { cx, cy, hr, hry } = b;
...
I('h6', 'hat', '海边草编帽', ['fresh', 'sea'], ['#e8c46a'],
  b => {
...
/* 头饰 5 */
I('hw1', 'headwear', '蝴蝶结发饰', ['cute', 'sweet'], ['#ff7eb6'],
  b => { const { cx, cy, hr, hry } = b; return bowSVG(cx + hr * 0.74, cy - hry * 0.6, 9 * b.fs, '#ff7eb6'); });
I('hw2', 'headwear', '花仙花环', ['fresh', 'princess'], ['#ff8fb0'],
  b => {
    const { cx, cy, hr, hry } = b;
...
I('hw3', 'headwear', '圆框眼镜', ['cool', 'basic'], ['#3a3a44'],
  b => {
...
I('hw4', 'headwear', '喵喵猫耳', ['cute', 'cool'], ['#4a4a58'],
  b => {
    const { cx, cy, hr, hry } = b;
    return [-1, 1].map(sd => {
...
I('hw5', 'headwear', '星空耳机', ['cool', 'sporty'], ['#2e2e3e'],
  b => {
    const { cx, cy, hr, hry, fs } = b;
...
/* 耳饰 5 */
I('e1', 'earrings', '珍珠耳钉', ['elegant', 'basic'], ['#fdf6ee'],
  b => {
    const { cx, cy, hr, hry } = b;
...
I('e4', 'earrings', '星星耳钉', ['cute', 'stage'], ['#ffe28a'],
  b => {
    const { cx, cy, hr, hry } = b;
    return [-1, 1].map(sd => star5(cx + sd * hr * 1.04, cy + hry * 0.42, 4.5 * b.fs, '#ffe28a')).join('');
  });
I('e5', 'earrings', '蜜桃心心坠', ['sweet', 'cute'], ['#ff7e9e'],
  b => {
    const { cx, cy, hr, hry } = b;
...
/* 项链 5 */
I('n1', 'necklace', '珍珠项链', ['elegant', 'basic'], ['#fdf6ee'],
  b => {
...
I('n2', 'necklace', '薄荷宝石链', ['fresh', 'sweet'], ['#5eead4'],
  b => {
    const { cx, shoulderY: sy, neckW } = b;
...
I('n5', 'necklace', '蝴蝶结颈链', ['cute', 'princess'], ['#ff7eb6'],
  b => {
    const { cx, shoulderY: sy, neckW } = b;
